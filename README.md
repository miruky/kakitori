...
判定のきびしさは「やさしい・ふつう・きびしい」の3段から選べ、許容するずれの幅が変わる。手本は「手本を再生」で一画ずつ書かれていく様子を見られる。練習しているモードと字はURLに残るので、特定の字の練習をそのまま共有・ブックマークできる。記録は「記録を消す」でいつでもまっさらにできる。
...
- `src/lib/judge.ts` 1画ぶんの手書き判定・不合格理由・きびしさ別のしきい値
- `src/lib/progress.ts` モード別の習熟記録
- `src/lib/url.ts` モードと練習中の字のURLハッシュ同期
- `src/main.ts` 手書きボード・モード切替・手本再生・字の一覧のUI
...
- **緩すぎず、厳しすぎず** — しきい値は「雑に書いても通る」と「丁寧に書いても落ちる」の間を実書きで調整した値を「ふつう」とし、やさしい・きびしいを加えた3段から選べる。値は1か所にまとめ、判定器に引数で渡す。
